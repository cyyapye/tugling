...
Stable promotion is intentionally separate from ordinary CI. The optional [release certification workflow](docs/release-certification.md) runs only after a maintainer explicitly enables it and approves a reviewed commit and token thresholds. It uses a dedicated API credential, the full behavioral matrix, and a separate attestation job. Ordinary PRs, pushes, and merges make no model calls; their real-CLI transport check uses a local fake API. Certification does not promote a release.

The existing promotion path still uses manually reviewed local evidence and a sanitized certificate committed with the candidate. It runs from a separately reviewed controller revision with the exact `main` SHA and certificate digest. Candidate verification has read-only repository permission; the write-enabled job executes only the pinned controller and treats the candidate as Git data. Connecting promotion to CI attestations remains a separate hardening step. See [Release controller](docs/release-controller.md) for bootstrap and recovery. Adopter repositories keep deterministic, model-free CI.
